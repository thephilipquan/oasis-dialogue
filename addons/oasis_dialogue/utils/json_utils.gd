...
static func parse_int(value: Variant) -> int:
	if value is String:
		return value.to_int()
	return int(value)
...
static func safe_get_int(json: Dictionary, key: String, default: int) -> int:
	var value: Variant = json.get(key, default)
	if is_int(value):
...
static func get_vector2(json: Dictionary, key: String, default: Vector2) -> Vector2:
	var type: Variant = json.get(key, {})

	if not type is Dictionary:
		return default
	var v: Dictionary = type

	var x: Variant = v.get("x", default.x)
	if is_int(x):
		x = parse_int(x)
	else:
		x = default.x

	var y: Variant = v.get("y", default.y)
	if is_int(y):
		y = parse_int(y)
	else:
		y = default.y
...
static func safe_get(json: Dictionary, key: String, default: Variant) -> Variant:
	var value: Variant = json.get(key, default)
